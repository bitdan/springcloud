...
public class StringUtils {
     ...
     *
     * @param str     字符串
     * @param onlyNum 是否仅为数字字符(不包含小数点、正负符号)
     * @return true:仅包含数字(或者小数点、正负符号)
     */
     ...
     * 字符转Unicode编码
     *
     * @param charArray char数组("".toCharArray())
     * @return Unicode编码
     */
	...
	public static String dataChange(String data) {
		...
		int hl = data.length()/2;
		for (int i = 0; i < hl; i++) {
			chars[i*2+1] = data.charAt(i);
		}
		for (int i = hl; i < data.length(); i++) {
			chars[(i-hl)*2] = data.charAt(i);
		}
		res = CharBuffer.wrap(chars).asReadOnlyBuffer().toString();
		//System.out.println(res);
		int datalf = (res.length()-totalTmp)/2;
		res = res.substring(5, 5+datalf) + res.substring(totalTmp+datalf);
		return res;
	}
	...
	public static String changeData(String data) {
		String result = "";
		int totalTmp = 8;
		data = ranStr(5) + data.substring(0, data.length() / 2) + ranStr(totalTmp-5) + data.substring(data.length() / 2);
